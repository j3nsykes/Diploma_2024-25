...
String gestureLabel[NUM_GESTURES] ;
float probablity[NUM_GESTURES] ;
    ...
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      ...
      if (numSamplesRead == NUM_SAMPLES) {
        ...
        for (int i = 0; i < NUM_GESTURES; i++) {
          ...
          probablity[i] = tflOutputTensor->data.f[i];
          gestureLabel[i] = GESTURES[i];
        }


//parse the data over Serial as JSON string for ease of reading in P5JS
        for (int i = 0; i < NUM_GESTURES; i++) {
          String json;
          json = "[";
          json = json + "{\"label\":";
          json = json + "\"";//need " " around gestureLabel so it is read in JSON as string
          json = json + gestureLabel[i];
          json = json + "\"";
          json = json + ",\"probability\":";
          json = json + probablity[i];
          json = json + ",\"winner\":";
          json = json + "\"";
          json = json + GESTURES[maxIndex];
          json = json + "\"";
          json = json + "}";
          json = json + "]";
          Serial.println(json);
          Serial.flush();
          delay(50);//CRUCIAL STABILITY!!!!!
        }
